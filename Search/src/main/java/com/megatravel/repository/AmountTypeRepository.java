...
public interface AmountTypeRepository extends JpaRepository<AmountType, Long> {
	...
	@Query("Select amt from AmountType amt where"
			...
			+ " unit.priceList.id in (Select p.id from PriceList p where "
			+ " p in (Select u from UnitPriceInformation u where u.room.id = ?1) AND sysdate() between p.beginDate and p.endDate)))")
